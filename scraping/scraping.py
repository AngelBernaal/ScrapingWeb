...
df['price'] = df['Price'].str.replace('$', '').str.replace(',', '').astype(float, errors='ignore').fillna(0)
df['rating'] = df['Rating'].str.extract(r'(\d+\.?\d*)').astype(float, errors='ignore').fillna(0)
print(df)
